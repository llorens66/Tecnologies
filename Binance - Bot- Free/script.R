...
freq <- "15m"           # frecuencia observaciones
...
strategy_func <- function(df){
  ...
  mutate(
    action = case_when(
      (lag(stoch_signal, 6) == 1 | lag(stoch_signal, 5) == 1 | lag(stoch_signal, 4) == 1 | lag(stoch_signal, 3) == 1 | lag(stoch_signal, 2) == 1 | lag(stoch_signal, 1) == 1 | stoch_signal == 1) &
        (lag(rsi_signal, 6) == 1 | lag(rsi_signal, 5) == 1 | lag(rsi_signal, 4) == 1 | lag(rsi_signal, 3) == 1 | lag(rsi_signal, 2) == 1 | lag(rsi_signal, 1) == 1 | rsi_signal == 1) & 
...
data1 <- data %>% map( ~ indicadors_func_safe(.)) %>% map( ~ strategy_func_safe(.)) %>% keep(.p = function(x){is.data.frame(x)})

# Cheackeamos criptos con senyal de macd
data1 %>% 
  keep(.p = function(x){
    tail <- x$macd_signal %>% tail(5)
    tail <- tail == 1 
    any(tail) == TRUE
  }) %>%
  map_df( ~ tail(.,5), .id = 'Activo') %>% 
  filter(Activo != 'BTTBTC') %>%
  select(Activo, dateTime, open, close, stoch_signal, rsi_signal, macd_signal, action) %>% 
  print()
...
print(trades_sumario$n[trades_sumario$result == 'tp_buy']*(tp*100)-trades_sumario$n[trades_sumario$result == 'sl_buy']*(sl*100))

# Buscamos si en este preciso momento hay alguna senyal de compra
trade_actual <- data1 %>% 
  keep(~ is.data.frame(.)) %>%
  map_df(.f = function(x){
  x %>% 
    tail(2) %>%
    head(1) %>%
    select(dateTime, open, close, stoch_signal, rsi_signal, macd_signal, action)
},.id = 'Activo') %>%
  filter(Activo != 'BTTBTC')
print(trade_actual)
...
trade_actual %>% 
  ...
  write_csv(file = 'historial_trade_actual.csv', append = TRUE)
...
if(!anyNA(trade_actual)){
  ...
  quantity <- quantity_func(trade_actual, eur_disp, max_decimals)
  print(quantity)
